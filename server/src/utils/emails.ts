...
export const sendRegistrationEmail = (email: string, token: string) => {
  ...
  const mailOptions = {
    ...
    html: getRegistrationEmailHTML(token),
  };
...
export const sendResetPassEmail = (
  ...
  const mailOptions = {
    ...
    html: getResetPasswordEmailHTML(userId, token),
  };
...
// Generate HTML for registration email
const getRegistrationEmailHTML = (token: string) => {
...
// Generate HTML for reset password email
const getResetPasswordEmailHTML = (userId: string, token: string) => {
  return `<div style="margin: 0px auto; max-width: 600px;">
  <table style="width: 100%; border-collapse: collapse;" role="presentation" border="0" cellspacing="0" cellpadding="0" align="center">
  <tbody>
  <tr>
